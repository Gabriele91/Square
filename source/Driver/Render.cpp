...
namespace Render
{
	...
	using square_render_create_context = Square::Render::Context* (*)(Square::Allocator*);
	...
	//alloc
	Context* create_render_driver(Allocator* allocator, RenderDriver type)
	{		
		//usings
		using namespace Square;
		using namespace Square::Filesystem;
		...
		for (auto file : get_files(location))
		{
			...
			if (auto lib = open(Filesystem::join(location, file)))
			{
				...
				{
					Context* context = create_context(allocator);
