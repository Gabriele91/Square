...
// Shader
// ──────────────────────────────────────────────────────────────────────────────
static size_t compute_buffer_size_with_padding(const MTLReflection& reflection)
{
    // Build the CPU-side auto buffer. The reflected members are tightly packed, but
    // the MSL struct generated by SPIRV-Cross for the loose-uniform buffer is aligned
    // to 16 bytes (it stores float4 values), so its sizeof() is rounded up to the
    // next multiple of 16. Metal validation requires the bound buffer to be at least
    // that size, so pad the allocation to a 16-byte boundary (the extra bytes remain
    // zero-initialized).

    // Compute buffer size
    uint32_t auto_sz = 0;
    for (auto& m : reflection.members)
        auto_sz = std::max(auto_sz, m.second.offset + m.second.size);

    // Round up to a 16-byte alignment.
    auto_sz = (auto_sz + 15u) & ~15u;
...
Shader* ContextMTL::create_shader(const std::vector<ShaderSourceInformation>& infos)
{
    ...
    for (const auto& info : infos)
    {
        ...
        // Compute buffer size for the uniforms
        const size_t buffer_size = compute_buffer_size_with_padding(sh->m_refl[stage]);

        // If there are stuff to uniform.... so alloc 
        if (buffer_size)
        {
            sh->m_auto_cpu[stage].resize(buffer_size, 0);
            sh->m_auto_gpu[stage] = [m_device newBufferWithLength:buffer_size
