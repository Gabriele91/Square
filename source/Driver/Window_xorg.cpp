...
namespace Video
{
	...
	struct wrapper_context
	{
		...
		std::map<WindowXorg*, WindowXorg*> m_windows;
		std::map<InputXorg*, InputXorg*>   m_input;
		//key map
		input_key_map			   m_key_map;
	};
	static wrapper_context s_os_context;
	...
	{
		//to 0
		std::memset(&s_os_context.m_xcontext, 0, sizeof(XContext));
		//ini key map
		helper_input_init();
		//init screen
		helper_screen_init();
	...
	void close()
	{
		...
		//close display
		XCloseDisplay(s_os_context.m_xdisplay);
		//to null
		s_os_context.m_xdisplay = nullptr;
	}
	...
	{
		auto* screen = (ScreenXorg*)m_native;;
		width = XDisplayWidth(s_os_context.m_xdisplay, screen->m_screen_id);
		height = XDisplayHeight(s_os_context.m_xdisplay, screen->m_screen_id);
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	static WindowXorg* x11_create_window(const WindowInfo& info)
	{
		XSetWindowAttributes win_attributes;
		...
		if(info.m_fullscreen)
		{
			...
			//end window
			wrapper_wnd = new WindowXorg(WindowXorg::GL_WINDOW, info, visual_info, wnd, desktop_info, xgl_ctx);
		}
		else 
		{
			x11_create_screen_window(info,  root_xwindow, visual_info, wnd);
			//end window
			wrapper_wnd = new WindowXorg(WindowXorg::GL_WINDOW, info, visual_info, wnd, xgl_ctx);
		}
		//get context
		wrapper_wnd->acquire_context();
		//return
		return wrapper_wnd;
	}
	//window
	static WindowXorg* window_create(const WindowInfo& info, Window* window)
	{
		if (info.m_context.m_type == ContextInfo::CTX_DIRECTX)
			return nullptr;
		//window
		WindowXorg* window_xorg = x11_create_window(info);
	...
	{
		//cast
		const auto* window = (const WindowXorg*)m_native;
		//
	...
	void Window::destoy()
	{
		...
		{
			//cast
			auto* window = (WindowXorg*)m_native;
	...
	{
		m_native = input_create((WindowXorg*)wnd->conteiner(), this);
	}
	...
	{
		//get n_events
		int n_events = XPending(s_os_context.m_xdisplay);
		//
		while (n_events--)
		{
			XEvent event;
			XNextEvent(s_os_context.m_xdisplay, &event);
			proc_xevent(event);
		}
	}
