...
namespace Square
{
	...
	*/
	...
	void CTangentSpaceCalculation<InputProxy>::AddNormal2Base( std::multimap<CBaseIndex,unsigned int,CBaseIndexOrder> &inMap,
															   const unsigned int indwPosNo, 
															   const unsigned int indwNormNo,
	...
	// ---------------------------------------------------------------------------------------------------------------
	struct ProxyTangentUtilsProxy : public ITriangleInputProxy
	{
		TangentUtils::Proxy& m_proxy;
		//
		ProxyTangentUtilsProxy(TangentUtils::Proxy& proxy):m_proxy(proxy){}
		//
		unsigned int GetTriangleCount() const { return m_proxy.triangles(); }
		//
		void GetTriangleIndices(const unsigned int indwTriNo, unsigned int outdwPos[3], unsigned int outdwNorm[3], unsigned int outdwUV[3]) const
		{
	...
	void TangentUtils::tangent_model_slow(TangentUtils::Proxy& model, bool replace_normal)
	{
		//compute
		CTangentSpaceCalculation<ProxyTangentUtilsProxy> tangents;
		bool success = tangents.CalculateTangentSpace(ProxyTangentUtilsProxy(model)) == 0;
		//get...
		for (unsigned int t = 0; t != model.triangles(); ++t)
		{
			unsigned int baseIndices[3];
			tangents.GetTriangleBaseIndices(t, baseIndices);
